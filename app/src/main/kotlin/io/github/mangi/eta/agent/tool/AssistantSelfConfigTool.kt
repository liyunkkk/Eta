...
internal class AssistantSelfConfigTool(
    ...
    fun updateAssistantConfig(args: JSONObject): String {
        ...
        if (targetLower.contains("key") || targetLower.contains("secret") ||
            ...
            return errorJson(
                "PERMISSION_DENIED",
                "出于最高安全防护原则，系统严禁通过自然语言对话修改或传入 API Key/Token 凭据。请在 Eta 设置界面中手动配置。"
            )
        }
    ...
    private fun handleToggleSetting(target: String, valueStr: String): String {
        val key = when (target.lowercase()) {
            ...
            "pure_mode", "overlay_pure_mode", "agent_overlay_pure_mode" -> Prefs.Keys.AGENT_OVERLAY_PURE_MODE
        ...
        if (key == null) {
            return errorJson(
                "UNSUPPORTED_SETTING",
                "不支持修改该配置项：$target。允许的受控开关：thinking_mode, pure_mode, terminal_tools, browser_tools, kimi_builtin_browser"
            )
        }
