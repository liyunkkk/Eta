                ...
                AgentMemoryScreen(
                    ...
                    onAction = { action ->
                        when (action) {
                            ...
                            is AgentMemoryAction.SelectSpace -> agentState.selectMemorySpace(action.space)
                            is AgentMemoryAction.SearchQueryChanged -> agentState.updateMemorySearchQuery(action.query)
                            AgentMemoryAction.OpenCreateCard -> agentState.openCreateMemoryCard()
                            is AgentMemoryAction.OpenEditCard -> agentState.openEditMemoryCard(action.card)
                            AgentMemoryAction.DismissCardDialog -> agentState.dismissMemoryCardDialog()
                            is AgentMemoryAction.SaveCard -> agentState.saveMemoryCard(action.card)
                            is AgentMemoryAction.DeleteCard -> agentState.deleteMemoryCard(action.id)
                            AgentMemoryAction.ClearAllCards -> agentState.clearAllMemoryCards()
